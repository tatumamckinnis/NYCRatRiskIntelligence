...
async def _bm25_retrieve(
    ...
    *,
    ...
    A ``ts_rank_cd`` normalization value of 5 (log-length + unique-word-
    count) was tried here to reward term density over raw length — it
    looked like a ~30% relative recall@k improvement in an isolated SQL
    test against the raw gold-set question text, but measured as a real
    regression (recall_at_k 0.31→0.21, faithfulness 0.72→0.58, both clean
    uncontaminated runs) once tested end-to-end through the actual /chat
    pipeline, which runs an LLM query-rewrite step before BM25 and feeds
    the result through RRF fusion + reranking — neither of which the
    isolated SQL test exercised. Reverted; default normalization (0) is
    the last known-good state. See [[project_ecb_penalty_chunking_2026_08_24]]
    for the full comparison. Lesson: validate retrieval ranking changes
    against the real pipeline (rewritten query, RRF, rerank), not a raw-SQL
    proxy — the same lesson the ECB chunking fix already taught once.
    """
    sql = """
        WITH q AS (
