...
faithfulness (via Groq judge).  Recall@k is reported when a trace JSONL
...
Usage::
    ...
    # With Groq API key for faithfulness judge
    GROQ_API_KEY=gsk_... uv run --package rat-evals python -m rat_evals.runners
...
# gold/ and results/ are two levels up from this file (evals/src/rat_evals/ → evals/src/ → evals/)
_EVALS_ROOT = Path(__file__).parent.parent.parent
_DEFAULT_GOLD = _EVALS_ROOT / "gold" / "article151_qa_v1.jsonl"
...
async def run_eval_suite(
    ...
    *,
    ...
    Args:
        ...
        api_key:     Groq API key for the faithfulness judge.
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Groq API key for faithfulness judge",
    )
